// ══════════════════════════════════════════════════════════════════
// edge-gateway — Fixed-window rate limiter adapter
//
// Implements the RateLimiter port using in-memory fixed-window algorithm.
// This is the adapter layer: concrete in-memory implementation behind an
// abstract interface.
//
...
/**
 * In-memory fixed-window rate limiter implementation of RateLimiter port.
 * Tracks request counts per bucket with 60-second windows.
 */
export class FixedWindowRateLimiter implements RateLimiter {
  ...
  check(key: string, limitPerMinute: number): RateLimitCheck {
    const now = this.#now();
    ...
    if (existing === undefined || now - existing.windowStartedAt >= WINDOW_MS) {
      ...
      return { allowed: true, remainingTokens: limitPerMinute - 1, retryAfterSeconds: 0 };
    }
    existing.count += 1;
...
export function assertWithinLimit(decision: RateLimitCheck): void {
