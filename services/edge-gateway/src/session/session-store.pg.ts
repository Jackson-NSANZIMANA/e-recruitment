// ══════════════════════════════════════════════════════════════════
// edge-gateway — Session store (PostgreSQL, rls/0019)
//
...
//   upstream_credential AES-256-GCM sealed (src/security/credential-cipher.ts).
//
...
export class PgEdgeSessionStore {
  ...
  #toSession(row: SessionRow, credential: string, idleExpiresAt: Date): EdgeSession {
    ...
    return {
      ...
      kind: row.kind as EdgeSessionKind,
  ...
  async create(input: CreateSessionInput, now: Date = new Date()): Promise<IssuedSession> {
  ...
  /**
   * Resolve a handle, classifying WHY it is unusable when it is.
   *
   ...
   */
  async resolve(handle: string, now: Date = new Date()): Promise<SessionLookup> {
  ...
  /**
   * Advance the idle window and ROTATE both secrets.
   *
   * The absolute ceiling is deliberately NOT advanced — the UI can then tell
   * the truth about when the session ends no matter how active the officer is.
   */
  async rotate(session: EdgeSession, now: Date = new Date()): Promise<IssuedSession> {
    const handle = newHandle();
    const csrfToken = newCsrfToken();
    const idleExpiresAt = this.#idleDeadline(now, session.absoluteExpiresAt);
    ...
    try {
      const row = await sql.begin(async (tx) => {
        ...
        const rows = await tx<SessionRow[]>`
          ...
          SET previous_handle_hash = handle_hash,
              ...
              previous_valid_until = ${graceUntil},
              handle_hash = ${this.#handleHash(handle)},
              csrf_token_hash = ${csrfTokenHash(this.#config.handleHmacKey, csrfToken)},
      ...
      if (row === null) {
        throw new EdgeSessionStoreError('Session is no longer refreshable.');
      }
      return {
        handle,
        csrfToken,
        session: this.#toSession(row, session.upstreamCredential, idleExpiresAt),
      };
    } catch (err) {
      if (err instanceof EdgeSessionStoreError) throw err;
      throw new EdgeSessionStoreError('Failed to rotate an edge session.', { cause: err });
    }
  }
  ...
  async stats(now: Date = new Date()): Promise<SessionStoreStats> {
    try {
      return await sql.begin(async (tx) => {
        await tx`SET LOCAL ROLE ${sql(EDGE_DB_ROLE)}`;
        const rows = await tx<
