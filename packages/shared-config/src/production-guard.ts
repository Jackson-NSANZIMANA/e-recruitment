...
// and every one of the 38 proofs are completely unaffected.
//
...
//
// SHA-256 of each secret literal in .env.example, NOT the literal itself.
...
const COMMITTED_DEV_SECRET_DIGESTS: ReadonlySet<string> = new Set([
  ...
  '941dbe652fb884249013c4ff6602eb68e70c40a56a7c9a9b30f5e5c92e352e31', // QR_SIGNING_KEY_ID
...
// ── Rule 2: placeholders ───────────────────────────────────────────
//
// MINIO_ENCRYPTION_KEY and EDGE_SESSION_HMAC_KEY are both still literal
// CHANGE_ME_32_CHAR_MIN_* on main. The first is required-to-boot and clears
// its 32-character floor, so it is a placeholder that WORKS — the worst
// kind. It would seal scanned national IDs under a key published in git.
...
export function assertProductionSecrets(source: EnvSource = process.env): void {
  ...
  for (const key of SECRET_KEYS) {
    ...
    if (COMMITTED_DEV_SECRET_DIGESTS.has(sha256(raw))) {
      issues.push(
        `${key} is still the well-known DEVELOPMENT value committed in .env.example — ` +
