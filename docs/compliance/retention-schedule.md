...
| Stored contact phone (ADR-021) | `applicant_identities.encrypted_phone_number` | Lifecycle-tied: lives exactly as long as the identity row's PII (rows above) | NULLed by the same gated tombstone — both roads (citizen-demand erasure AND retention sweep share one ErasureRepository) | **Built + proven (gate #36)** |
